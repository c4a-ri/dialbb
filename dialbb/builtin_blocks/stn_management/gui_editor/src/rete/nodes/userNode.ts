...
export class userNode extends Classic.Node<
  ...
  constructor(
    ...
    this.addControl(
      "seqnum",
      new CustomInputControl("number", "", { initial: seqnum })
    );
    // ユーザ発話
    this.addControl(
      "utterance",
      new CustomInputControl("text", "user utterance example", { initial: userUtter })
    );
    // ユーザ発話タイプ
    this.addControl(
      "type",
      new CustomInputControl("text", "user utterance type", { initial: type })
    );
    // 条件
    this.addControl(
      "conditions",
      new CustomInputControl("text", "conditions", { initial: conditions })
    );
    // アクション
    this.addControl(
      "actions",
      new CustomInputControl("text", "actions", { initial: actions })
    );
    // 遷移する状態
    this.addControl(
      "nextStatus",
      new CustomInputControl("text", "", { initial: nextSt }, true)
    );
