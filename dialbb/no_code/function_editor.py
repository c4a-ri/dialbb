...
def edit_scenario_functions(parent, file_path: str):
    ...
    toplevel.title(gui_text("func_edt_title"))
    toplevel.grab_set()  # モーダルにする
    toplevel.focus_set()  # フォーカスを新しいウィンドウをへ移す
    toplevel.transient(parent)
    # サイズ＆表示位置の指定
    parent_x = (
        parent.winfo_rootx() + parent.winfo_width() // 4 - toplevel.winfo_width() // 2
    )
    parent_y = (
        parent.winfo_rooty()
        + parent.winfo_height() // 10
        - toplevel.winfo_height() // 2
    )
    ...
    label = tk.Label(toplevel, text=gui_text("func_edt_funcs"))
    textarea = scrolledtext.ScrolledText(toplevel, wrap=tk.NONE, width=80, height=30)
    # 横方向のスクロールバーを作成
    horiz_scrollbar1 = tk.Scrollbar(
        toplevel, orient=tk.HORIZONTAL, command=textarea.xview
    )
    textarea.config(xscrollcommand=horiz_scrollbar1.set)
    # configの値を設定
    textarea.insert(0.0, manager.get_scenario_functions())
    ...
    # OKボタン
    ok_btn = ttk.Button(
        toplevel, text=gui_text("btn_ok"), command=lambda: ok_btn_click()
    )
    # Cancelボタン
    cancel_btn = ttk.Button(
        toplevel, text=gui_text("btn_cancel"), command=lambda: cancel_btn_click()
    )
