...
- RAGのパッセージ検索ブロックを使う場合は，文書ローダ向けのオプション依存も追加でインストールしてください．

  ```sh
  $ pip install '<ダウンロードしたwhlファイル>[rag]'
  ```
...
パッセージ検索ブロックには，文書ローダ向けのオプション依存も必要です．ベースのwheelだけをインストールした場合は，追加で `pip install 'dialbb[rag]'` を実行してください．
