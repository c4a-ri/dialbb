...
- もし，すでにPythonがインストールされていれば，バージョンを確かめてください．インストールされていても，バージョンが3.11-3.14であれば，新しくインストールする必要はありません．3.10以前のPythonがインストールされている場合，使っていないならアンインストールしてください．使っている場合，DialBBの開発者（dialbb あっと c4a.jp）に相談してください．
...
- Windows 11
  ...
  - Pythonがインストールされていない場合，以下の手順でPythonをインストールします．
    ...
    - 表示されている中から以下の部分を探します．以下は3.11.9ですが，3.11.x-3.14.xであれば，別のバージョンでも構いません．**DialBB 2.0では3.11.x-3.14.xを利用してください．3.10.x以前と3.15.x以降は利用できません**．
...
| モデル | 利用する大規模言語モデル（LLM）を選択します．`その他`を押すと，プルダウンにないモデルを追加できます． |
...
スロット抽出とは，それまでの対話から必要な情報を取り出すことです．必要な情報のことをスロットと言います．例えば，気象情報を検索する対話システムならば，「地名」や「日付」などがスロットにになります．
...
スロット抽出知識のExcelファイルは，`dialogues`シートとslots`シートからなります．
