...
- Utterance canonicalizer: 正規化ブロック．ユーザ入力文の正規化（大文字→小文字，全角⇒半角の変換など）を行います．
...
| ファイル名               | 説明                                                         |
| ------------------------ | ------------------------------------------------------------ |
| config.yml               | アプリケーションを規定するconfigurationファイルです。どのようなブロックを使うかや、各ブロックが読み込むファイルなどが指定されています。このファイルのフォーマットは{ref}`configuration`で詳細に説明します。 |
