...
- 利用できるプレースホルダ
  ...
  - {aux_dataのキー}
    ...
    補助データのキーの値（文字列ではない場合は文字列に変換する）で置き換えられる

- 置き換えられなかったプレースホルダの消去

  `[[[`と`]]]`で囲まれた部分は、置き換えられなかったプレースホルダが残っているとき、プロンプトから消去されます。例えば、
  ...
  のような部分がプロンプトテンプレートにあったとき、`aux_data`に`caution`がなければ、この部分は削除されます。なお、このとき、aux_dataのキーはアルファベット、数字、アンダースコアのみからなる必要があります。
...
- `{current_time}`は現在の日付、時刻で置き換えられます。コンフィギュレーションファイルの`language`の値が`ja`なら日本語、それ以外は英語の文字列になります。

  ```
  現在の時刻が{current_time}であることを考慮に入れて発話を生成してください。
  ```

  のような形で使えます。

- `{aux_dataのキー}`は補助データの値で置き換えられます。値が文字列でないときは文字列に変換されます。

- `[[[`と`]]]`で囲まれた部分は、置き換えられなかったプレースホルダが残っているとき、プロンプトから消去されます。
