...
- 出力
  ...
  - `aux_data`: 補助データ（辞書型）(ver. 0.3.1で変更）
...
- `repeat_when_no_available_transitions` （ブール値．デフォルト値`False`．ver. 0.3.1で追加）
...
- `#prep`
  ...
  `#prep`からの遷移先は`#initial`でなくてもよくなりました．(ver. 0.3.1)
...
STN Managerは，対話のセッションごとに**文脈情報**を保持しています．文脈情報は変数とその値の組の集合（pythonの辞書型データ）で，値はどのようなデータ構造でも構いません．
...
ver. 0.3.1で，音声認識結果を入力として扱うときに生じる問題に対処するため，以下の処理が導入されました．

#### ブロックコンフィギュレーションパラメータ
...
- `utterance_to_ask_repetition`（文字列）
   ...
   これが指定されている場合，入力の確信度が低いときは，状態遷移をおこなわず，"utterance_to_ask_repetition"の値をシステム発話とします．ただし，バージインの場合（`aux_data`に`barge_in`要素があり，その値が`True`の場合）を除きます）
