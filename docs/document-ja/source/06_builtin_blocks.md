...
`_check_with_llm(task)`および`_generate_with_llm(task)`は、大規模言語モデル（現在はOpenAIのChatGPTのみ）を用いて条件の判定および文字列の生成を行います。



これらの関数を使うためには、環境変数`OPENAI_API_KEY`にOpenAIのAPIキーをセットするする必要があります。
