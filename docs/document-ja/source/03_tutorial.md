...
DialBBにはいくつかのサンプルアプリケーションが付属しています．本章ではこれらを用いてDialBBのアプリケーションの構成と、DialBBを用いてアプリケーションを構築する方法を説明します。
...
blocks:
  ...
  - name: parrot
    ...
    input:
      input_text: user_utterance  # argument for this block : attribute name in the blackboard
...
`sample_apps/simple_ja/`に日本語版が，`sample_apps/simple_en/`に英語版があります．
