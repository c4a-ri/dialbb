...
- プロンプト内のプレイスホルダに関するバグの修正

## 2.1.0 ()

- シミュレータ

  dialbbアプリケーションをシミュレータとして使えるように

  LLMシミュレータのLLM呼び出しを変更
...
- 以下のブロックを非推奨
  ...
  - ロジスティック回帰と条件付き確率場を用いた言語理解ブロック

  - ChatGPTを用いた言語理解ブロック

  - ChatGPTを用いた固有表現抽出ブロック
...
- STN Manager組み込みブロック
  ...
  - LLM呼び出しをLangChainで行うように変更
  
  - dialogue_historyをinputとして取れるように変更

- ノーコードツール

  言語理解、固有表現抽出の代わりにLLMベースのDSTを使えるように変更

- poetryからuvベースに変更

- デフォルトのLLMをgpt-5.4-nanoに変更

- context DBを使わないとき，自動生成するsession_idを短い文字列にする．

- .envから環境変数設定を読み込めるように変更
