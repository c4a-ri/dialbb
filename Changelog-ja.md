...
- ノーコードツール
  ...
  - APIキーを設定したときにすぐに反映されない場合がある問題を解決
...
- ChatGPT対話組み込みブロックを非推奨

- 日本語文字列正規化ブロックを非推奨

- 単純文字列正規化ブロックを非推奨

- ロジスティック回帰と条件付き確率場を用いた言語理解ブロックを非推奨

- ChatGPTを用いた言語理解ブロックを非推奨

- ChatGPTを用いた固有表現抽出ブロックを非推奨

- spaCyを用いた固有表現抽出ブロックを非推奨

## 1.2.3 (2026.5.16)

- ノーコードツール

  - シナリオエディタで保存してすぐに反映されないバグをfix

  - シナリオエディタで整列した際にユーザノードが優先度順に並ばないバグをfix
...
- ノーコードツール
  ...
  - 言語理解、固有表現抽出が使えないバグをフィックス

## 1.2.1 (2026.4.21)

- シミュレーションベーステスタのバグフィックス

- ノーコードツール

  - シミュレーションベーステストの最大ターン数を指定可能に
  
  - シナリオエディタのキャンバスの大きさを自動で変更
...
- STN Manager組み込みブロック
  ...
  - ChatGPTを使って発話生成と条件判断をさせる場合、プロンプトテンプレートを指定できるように変更

  - シンタクスシュガー `$"..."`の代わりに`$...$`も使えるように変更
